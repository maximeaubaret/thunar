...
thunar_window_trash_infobar_clicked (GtkInfoBar   *info_bar,
                                     ...
                                     ThunarWindow *window);
static void
thunar_window_update_image_preview (ThunarWindow *window);
static void
...
thunar_window_class_init (ThunarWindowClass *klass)
{
  ...
  /**
   * ThunarWindow:searching:
   *
   * Whether the active view is currently searching files.
   **/
  g_object_class_install_property (gobject_class,
                                   PROP_SEARCHING,
                                ...
                                GtkWidget    *menu)
{
  ...
  if (THUNAR_IS_STANDARD_VIEW (window->view))
    ...
    {
      thunar_miller_columns_view_append_menu_item (THUNAR_MILLER_COLUMNS_VIEW (window->view),
                                                   GTK_MENU (menu), THUNAR_MILLER_COLUMNS_VIEW_ACTION_SELECT_ALL_FILES);
      thunar_miller_columns_view_append_menu_item (THUNAR_MILLER_COLUMNS_VIEW (window->view),
                                                   GTK_MENU (menu), THUNAR_MILLER_COLUMNS_VIEW_ACTION_SELECT_BY_PATTERN);
      thunar_miller_columns_view_append_menu_item (THUNAR_MILLER_COLUMNS_VIEW (window->view),
                                                   GTK_MENU (menu), THUNAR_MILLER_COLUMNS_VIEW_ACTION_INVERT_SELECTION);
      thunar_miller_columns_view_append_menu_item (THUNAR_MILLER_COLUMNS_VIEW (window->view),
                                                   GTK_MENU (menu), THUNAR_MILLER_COLUMNS_VIEW_ACTION_UNSELECT_ALL_FILES);
    }
  xfce_gtk_menu_append_separator (GTK_MENU_SHELL (menu));
          ...
          if (trash_folder != NULL)
            {
              ...
              if (action_entry != NULL)
                {
                  if (thunar_file_get_trash_item_count (trash_folder) > 0)
                            ...
                            GParamSpec *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_SEARCHING:
      g_value_set_boolean (value, window->searching);
      break;
                            ...
                            GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_SEARCHING:
      window->searching = g_value_get_boolean (value);
      break;
                                   ...
                                   ThunarWindow *window)
{
  ...
  if (window->view_type == G_TYPE_NONE)
    window->view_type = G_OBJECT_TYPE (view);
}
                                     ...
                                     ThunarWindow *window)
{
  ...
  /* drop connected signals from the view */
                                    ...
                                    GtkWidget    *view)
{
  ...
  /* Create tab content - always use paned container */
  ...
  if (saved_height <= THUNAR_TERMINAL_MIN_TERMINAL_HEIGHT)
    saved_height = THUNAR_TERMINAL_MIN_TERMINAL_HEIGHT;
#else
  /* Without VTE: Add placeholder widget */
  {
    GtkWidget *placeholder = gtk_box_new (GTK_ORIENTATION_HORIZONTAL, 0);
    gtk_paned_pack2 (GTK_PANED (tab_content_paned), placeholder, FALSE, TRUE);
    gtk_widget_hide (placeholder); /* Hide placeholder by default */
  }
#endif
...
{
  gboolean   response;
  GtkWidget *tab_page_to_destroy = NULL;

  if (window->view != NULL)
    {
      /* Get the top-level container for the tab page, which is the GtkPaned. */
      tab_page_to_destroy = gtk_widget_get_parent (window->view);

      /* Sanity check that the parent is indeed a paned widget */
      if (!GTK_IS_PANED (tab_page_to_destroy))
        {
          g_warning ("Could not find the parent GtkPaned for the current view. Aborting tab close.");
          return TRUE;
        }
    }
  ...
  if (thunar_window_split_view_is_active (window))
    {
      if (tab_page_to_destroy != NULL)
        /* Destroying the tab's container will trigger page-removed and all other cleanup */
  ...
  else
    {
      if (window->preview_image_pixbuf != NULL)
      ...
      else
        {
          ...
          gtk_widget_hide (window->right_pane_size_value);
        }
    }
}
