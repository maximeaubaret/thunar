                                ...
                                gpointer      user_data)
{
  ...
  else if (type == THUNAR_TYPE_MILLER_COLUMNS_VIEW)
                                ...
                                gpointer      user_data)
{
  switch (g_value_get_int (src_value))
    ...
    case 3:
      g_value_set_static_string (dst_value, g_type_name (THUNAR_TYPE_MILLER_COLUMNS_VIEW));
      break;
...
thunar_preferences_dialog_init (ThunarPreferencesDialog *dialog)
{
  ...
  gtk_combo_box_text_append_text (GTK_COMBO_BOX_TEXT (combo), _("Miller Columns"));
  ...
  g_object_bind_property (G_OBJECT (dialog->preferences),
                          ...
                          G_BINDING_BIDIRECTIONAL | G_BINDING_SYNC_CREATE);
  gtk_widget_set_tooltip_text (button,
                               _("Select this option to remember view type, zoom level and sort settings individually for each folder"));
  ...
  label = gtk_label_new (_("Mouse Interactions"));
  gtk_label_set_attributes (GTK_LABEL (label), thunar_pango_attr_list_bold ());
  gtk_frame_set_label_widget (GTK_FRAME (frame), label);
  gtk_widget_show (label);
  ...
  gtk_widget_set_tooltip_text (combo, _("Disabled:\n"
                                        ...
                                        "\t- Show a menu to select an action.\n"
                                        "\n"
                                        "Keyboard-shortcuts to select the action ahead of the drop:\n"
                                        "\t- [Ctrl]: Copy\n"
                                        "\t- [Shift]: Move\n"
                                        "\t- [Ctrl]+[Shift]: Create a Link"));
  gtk_widget_set_hexpand (combo, TRUE);
  gtk_grid_attach (GTK_GRID (grid), combo, 1, row, 1, 1);
  thunar_gtk_label_set_a11y_relation (GTK_LABEL (label), combo);
  gtk_label_set_mnemonic_widget (GTK_LABEL (label), combo);
